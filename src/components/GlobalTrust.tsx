...
function StatCard({
  ...
  return (
    ...
    >
      ...
      >
        {count}
        <span style={{ color, fontSize: "0.7em", marginLeft: 2 }}>
          {suffix}
        </span>
      </div>
      <div
  ...
  useEffect(() => {
    ...
    function draw() {
      ...
      connections.forEach(([ai, bi], ci) => {
        ...
        // Moving dot
        const px =
          (1 - progress) * (1 - progress) * ax +
          2 * (1 - progress) * progress * mx +
          progress * progress * bx;
        const py =
          (1 - progress) * (1 - progress) * ay +
        ...
        >
          {stats.map((s, i) => (
            <motion.div key={s.label} variants={fadeUp}>
              <StatCard {...s} index={i} started={started} />
            </motion.div>
          ))}
        </motion.div>
      </div>
